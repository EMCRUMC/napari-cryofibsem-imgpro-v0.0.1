...
def process_slice(slice_data, dec_num, sigma, pad_width, pad_mode):
    # Pads original image
...
def decurtain(
    ...
    if len(image.data.shape) > 2:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_to_slice = {
                executor.submit(process_slice, stack[slice_idx], dec_num, sigma, pad_width_int, pad_mode):
        ...
        processed_stack = process_slice(image.data, dec_num, sigma, pad_width_int, pad_mode)

    print("Image or Stack was decurtained successfully!")

    image_name = f"Dcur_decnum{dec_num}_sigma{sigma}_pad{pad_width}{pad_mode}"

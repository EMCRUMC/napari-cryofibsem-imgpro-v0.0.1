...
def process_slice(slice_data, sigma_x, sigma_y, pad_width, pad_mode):
    # Pads original image
...
def decharge(
    ...
    if len(image.data.shape) > 2:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_to_slice = {
                executor.submit(process_slice, stack[slice_idx], sigma_x, sigma_y, pad_width_int, pad_mode):
        ...
        processed_stack = process_slice(image.data, sigma_x, sigma_y, pad_width_int, pad_mode)

    image_name = f"Dcha_sigmaX{sigma_x}_sigmaY{sigma_y}_pad{pad_width}{pad_mode}"
